...
enum {
	MaxGThreads = 8,            // Maximum number of threads, used as array size for gttbl
...
struct gt {
	...
	int exit_code;
	int priority;
	int starvation_boost;
	uint64_t starvation_rounds;
...
void gt_set_scheduler(enum gt_sched_policy policy);                      // select scheduling policy (RR/PRI/LS)
...
int gt_create(gt_entry_fn_t entry, void *arg, int priority);             // create new thread and set entry function + argument + base priority
