...
/* Pri vypise stromu preskoci specialne self/parent polozky. */
static int is_dot_or_dotdot(Fat16Entry *entry)
{
    return (entry->filename[0] == '.');
}

/* Vypise odsadenie dvoma medzerami pre uroven hlbky stromu. */
...
/* Vykresli jeden riadok uzla stromu ako DIR/FILE s volitelnou priponou. */
...
/* Rekurzivne prejde adresarove polozky a vypise cely podstrom. */
...
/* Verejny vypis stromu zacinajuci od rootu filesystemu. */
void printTree(void)
{
    print_string("/\n");
    print_tree_dir(0, 1);
