...
/* Streamuje obsah suboru z FAT retazca clusterov na konzolovy vystup. */
void read_file(char *filename)
{
    ...
    while (cluster < 0xFFF8)
    {
        ...
        for (unsigned int s = 0;s < bs.sectors_per_cluster && remaining;s++)
        {
            read_sector(first_sector + s, sector_buffer);

            unsigned int to_write = remaining < bs.sector_size ? remaining : bs.sector_size;
            console_write((char *)sector_buffer,to_write);
            remaining -= to_write;
            if (!remaining)
                return;
        }
